...
class LessonsController extends Controller
{
    ...
    public function convertToIframeLink($driveUrl)
    {
        // Check if the URL contains the Google Drive file format
        if (preg_match('/drive\.google\.com\/file\/d\/([a-zA-Z0-9_-]+)/', $driveUrl, $matches)) {
            // Extract the file ID
            $fileId = $matches[1];

            // Return the embeddable iframe link
        ...
        if (is_array($request->videos) && collect($request->videos)->isNotEmpty()) {
            foreach ($request->videos as $video) {
                ...
                if (isset($video['title'], $video['url'], $video['duration'])) {
                    $iframeUrl = $this->convertToIframeLink($video['url']);
